...
const double ER2 = 2.0 * 2.0; // ER*ER Radio de Escape
...
static inline void render(unsigned char *image, int width, int height, double xmin, double ymin, double xmax, double ymax, complex double X, complex double Y) {
  int times_iter;
  ...
  #pragma omp parallel for schedule(dynamic, 1)
    for (int j = 0; j < height; ++j) {
      for (int i = 0; i < width; ++i) {
        ...
        complex double a = -X / 3.0 + Y / 3.0;
        complex double v = 2.0 * X / 3.0 + Y / 3.0;
        complex double z = x + I * y;

        // check if its inside
        // complex double c = -1.07+0.12*I;
        int k=0;
        for (k=0; k<10 * UCHAR_MAX; k++) {
          for (int m=0; m<times_iter; m++) z = (z*z)*z - 3*z*(a*a) + 2*a*(a*a) + v;
          if (cabs2(z) > 4) break;
        }
        // done checking
        // k for channel h;
        color_tricorn(image, width, i, j, 10 * UCHAR_MAX - k); // Can add iAngle
      }
    }
}
...
int main() {
  ...
  //
      ...
      fscanf(input_file, "%lf", &x_x);
      fscanf(input_file, "%lf", &x_y);
      fscanf(input_file, "%lf", &y_x);
      fscanf(input_file, "%lf", &y_y);
      fclose(input_file);
  //
  //
  const char *filename = "./img/julia.ppm";
  unsigned char *image = image_new(width, height);
  render(image, width, height, xmin, ymin, xmax, ymax, x_x+I*x_y, y_x+I*y_y);
